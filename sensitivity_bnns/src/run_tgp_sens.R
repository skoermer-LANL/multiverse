...
# Load data
input_path <- file.path("..", "..", "results", paste0(method, "_", dgm, "_", "rev1"), "merged_results.csv")
output_path <- file.path("..", "..", "results", paste0(method, "_", dgm, "_", "rev1"), paste0("tgp_", response_col, "_sens.RData"))
...
lhs_path <- file.path("..", "..", "lhs", paste0(method, "_", dgm, "_lhs_rev1.csv"))
...
BTE <- c(5000, 150000, 10)

if(tgpsens){
sensfit <- sens(X = X, Z = Y, model = btgp, nn.lhs = 7000, BTE = BTE,
                rect = rect,
                pred.n = FALSE,
                improv = FALSE,
                krige = FALSE,
